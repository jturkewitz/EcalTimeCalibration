...
//    cryCalib->filterOutliers(); //commented out August 5th 2011
        ...
        timeItr != times.end(); ++timeItr)
    {
      ...
      if(isEB)
      {
        ...
        if(fabs(eta) >= 0.0 && fabs(eta) < 0.3)
        {
          etaSlicesTimingHistsEB_[0]->Fill(timeItr->time,weight);
        }
        if(fabs(eta) >= 0.3 && fabs(eta) < 0.6)
        {
          etaSlicesTimingHistsEB_[1]->Fill(timeItr->time,weight);
        }
        if(fabs(eta) >= 0.6 && fabs(eta) < 0.9)
        {
          etaSlicesTimingHistsEB_[2]->Fill(timeItr->time,weight);
        }
        if(fabs(eta) >= 0.9 && fabs(eta) < 1.2)
        {
          etaSlicesTimingHistsEB_[3]->Fill(timeItr->time,weight);
        }
        if(fabs(eta) >= 1.2 && fabs(eta) < 1.5)
        {
          etaSlicesTimingHistsEB_[4]->Fill(timeItr->time,weight);
        }
...
//  superModuleTimingMeanEB_ = fileService_->make<TGraph>("superModuleTimingMeanEB","Mean Time of EB SM [ns] ",36);
  ...
  etaSlicesTimingHistsEB_[0] = etaSlicesDir.make<TH1F>("EB_cryTiming_eta_0.0-0.3","EB_cryTiming_eta_0.0-0.3",200,-10,10);
  etaSlicesTimingHistsEB_[1] = etaSlicesDir.make<TH1F>("EB_cryTiming_eta_0.3-0.6","EB_cryTiming_eta_0.3-0.6",200,-10,10);
  etaSlicesTimingHistsEB_[2] = etaSlicesDir.make<TH1F>("EB_cryTiming_eta_0.6-0.9","EB_cryTiming_eta_0.6-0.9",200,-10,10);
  etaSlicesTimingHistsEB_[3] = etaSlicesDir.make<TH1F>("EB_cryTiming_eta_0.9-1.2","EB_cryTiming_eta_0.9-1.2",200,-10,10);
  etaSlicesTimingHistsEB_[4] = etaSlicesDir.make<TH1F>("EB_cryTiming_eta_1.2-1.5","EB_cryTiming_eta_1.2-1.5",200,-10,10);
